...
def process_tweets(csv_file,vocab, test_file=True):
    tweets = []
    labels = []
    ...
    with open(csv_file, 'r') as csv:
        ...
        for i, line in enumerate(lines):
            ...
            feature_vector = get_feature_vector(tweet,vocab)
            if test_file:
...
class DataGenerator(keras.utils.Sequence):
    def __init__(self,dlines,vocab,max_length,batch_size=32,shuffle=True,test_file=False):
        print('Generates data for Keras')
        print('Initialization')
        self.batch_size = batch_size
        self.dlines=dlines
        self.total=len(dlines)
        self.vocab=vocab
        self.max_length=max_length
        self.list_IDs = [i for i in range(0,self.total)]
        print(len(dlines))
        print(max(self.list_IDs))
        self.test_file=test_file
        self.shuffle = shuffle
        self.on_epoch_end()

    def __len__(self):
        #print('Denotes the number of batches per epoch')
    ...
    def __data_generation(self, list_IDs_temp):
        #print('Generates data containing batch_size samples') # X : (n_samples, *dim, n_channels)
        #process tweet data:
        tmp_lines=[self.dlines[id] for id in list_IDs_temp]
        tweets=[]
        labels=[]
        for i, line in enumerate(tmp_lines):
            if self.test_file:
                ...
                tweet_id, sentiment, tweet = line.split(',')
            feature_vector = get_feature_vector(tweet,self.vocab)
            if self.test_file:
                tweets.append(feature_vector)
            else:
                tweets.append(feature_vector)
                labels.append(int(sentiment))
        tweets = pad_sequences(tweets, maxlen=self.max_length, padding='post')
...
if __name__ == '__main__':
    ...
    with open(TRAIN_PROCESSED_FILE, 'r') as csv:
        lines = csv.readlines()
        total=len(lines)
    train_lines=lines[0:int(0.9*total)]
    val_lines=lines[-int(0.1*total):]
    train_generator=DataGenerator(train_lines, batch_size=256,shuffle=True,test_file=False,vocab=vocab,max_length=max_length)
    val_generator=DataGenerator(val_lines, batch_size=256,shuffle=True,test_file=False,vocab=vocab,max_length=max_length)
    ...
    if train:
        ...
        model.fit_generator(generator=train_generator,epochs=50,
                    validation_data=val_generator,workers=4, use_multiprocessing=True)
    else:
        model = load_model(sys.argv[1])
        print(model.summary())
        test_tweets, _ = process_tweets(TEST_PROCESSED_FILE,vocab, test_file=True)
